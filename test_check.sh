...
# No args → ft_ls should list "." (current directory), matching real ls.
expected=$(LC_ALL=C ls 2>/dev/null)
out=$(LC_ALL=C $FT_LS 2>/dev/null)
check_stdout "no args → same output as 'ls'" "$expected" "$out"
...
# Single directory: no header — contents should match real ls.
expected=$(LC_ALL=C ls "$TESTDIR" 2>/dev/null)
out=$(LC_ALL=C $FT_LS "$TESTDIR" 2>/dev/null)
check_stdout "single dir → no header" "$expected" "$out"
...
DIRA="$TESTDIR"
DIRB="$TESTSUBDIR"
# Two dirs → compare output against real ls with the same args.
expected=$(LC_ALL=C ls "$DIRA" "$DIRB" 2>/dev/null)
out=$(LC_ALL=C $FT_LS "$DIRA" "$DIRB" 2>/dev/null)
check_stdout "two dirs → header on each, blank between" "$expected" "$out"

# File then directory: compare output against real ls.
expected=$(LC_ALL=C ls "$TESTFILE_A" "$TESTDIR" 2>/dev/null)
out=$(LC_ALL=C $FT_LS "$TESTFILE_A" "$TESTDIR" 2>/dev/null)
check_stdout "file + dir → file first (no header), blank, dir with header" "$expected" "$out"

# Directory then file in argv: ls still prints file first — compare to real ls.
expected=$(LC_ALL=C ls "$TESTDIR" "$TESTFILE_A" 2>/dev/null)
out=$(LC_ALL=C $FT_LS "$TESTDIR" "$TESTFILE_A" 2>/dev/null)
check_stdout "argv dir then file → file still printed first" "$expected" "$out"

# Two files: no headers, sorted alphabetically — compare to real ls.
expected=$(LC_ALL=C ls "$TESTFILE_B" "$TESTFILE_A" 2>/dev/null)
out=$(LC_ALL=C $FT_LS "$TESTFILE_B" "$TESTFILE_A" 2>/dev/null)
check_stdout "two files → alphabetical order, no headers" "$expected" "$out"

# Three arguments: two files + one dir — compare to real ls.
expected=$(LC_ALL=C ls "$TESTFILE_B" "$TESTDIR" "$TESTFILE_A" 2>/dev/null)
out=$(LC_ALL=C $FT_LS "$TESTFILE_B" "$TESTDIR" "$TESTFILE_A" 2>/dev/null)
...
# A real path after "--" must work — compare output to real ls.
expected=$(LC_ALL=C ls -- "$TESTDIR" 2>/dev/null)
out=$(LC_ALL=C $FT_LS -- "$TESTDIR" 2>/dev/null); code=$?
